...
export const MemberCard = ({members}:any) => {
    ...
    return (<>
        {members && members.map((member: { id: number, firstname: string, lastname: string, dmn: string, image: string, payment_status: string, additional: string, nia: string, lead_position:string }) =>
            <div className="w-[170px] h-[230px] md:h-[200px] md:w-[200px] rounded-md  mx-auto shadow-sm bg-gradient-to-tl from-white to-slate-50 || dark:bg-gradient-to-tl dark:from-gray-900 dark:to-slate-900 cursor-pointer" key={member.id}>
                <div className="flex">
                <div className="absolute bg-teal-700 rounded-sm px-1 rotate-90 mt-4">
                    <p className="text-right text-white justify-end float-right text-[10px]">{member.lead_position  ?member.lead_position : null}</p>
                </div>
                
                <div className="absolute bg-pink-700 rounded-sm px-1 mx-28 md:mx-36 rotate-90 mt-4">
                ...
                <div className="px-4 text-slate-600 mt-4 text-center md:text-left font-light || dark:text-slate-400">
                    ...
                    <span className="text-black || dark:text-white">Name:</span> {member.firstname.toLowerCase()} {member.lastname}
                    </p>
                    <p className="text-xs">
